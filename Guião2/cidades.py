...
class Cidades(SearchDomain):
    ...
    def cost(self, city, action):
        ...
        for (origin,destiny,path_cost) in self.connections:
            if (origin==C1 and destiny==C2) or (origin==C2 and destiny==C1):
                return path_cost
        return None
    def heuristic(self, city, goal_city):
        distance = sqrt((self.coordinates[city][0]-self.coordinates[goal_city][0])**2 + (self.coordinates[city][1]-self.coordinates[goal_city][1])**2)
        return distance
