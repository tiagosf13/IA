...
class BayesNet:
    ...
    def individualProb(self, var, value):
        ...
        return sum([ self.jointProb([(var, value)] + conj) for conj in self.generate_conjunctions(variaveis) ])
    
    def generate_conjunctions(self, variaveis):
        if len(variaveis) == 1:
            return [ [(variaveis[0], True)] , [(variaveis[0], False)] ]

        l = []
        for c in self.generate_conjunctions(variaveis[1:]):
